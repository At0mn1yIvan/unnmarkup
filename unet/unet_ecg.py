...
class ECGDataset(Dataset):
    def __init__(
        ...
        self.max_start_idx_fragment = self.signal_len_points - (2 * 500) - self.fragment_len_points
    ...
    def _create_full_mask(self, annotation_symbols, annotation_samples):
        ...
        while i < len(annotation_symbols):
            if annotation_symbols[i] == "(":
                ...
                if i + 2 < len(annotation_samples) and annotation_symbols[i + 2] == ")":
    ...
    def _load_and_process_data(self):
        print(f"Загрузка и обработка данных для {'обучения' if self.is_train else 'валидации'}...")
        for record_name in tqdm(self.record_names, desc="Обработка записей ludb"):
            record_path = os.path.join(self.data_dir_path, record_name)
            try:
                # Загружаем все 12 отведений сразу
                signals_all_leads, meta = wfdb.rdsamp(record_path)  # signals_all_leads: (5000, 12)
                if signals_all_leads.shape != (12, 5000):
                    signals_all_leads = signals_all_leads.T  # signals_all_leads: (12, 5000)
                ...
                for lead_idx, lead_name_lower in enumerate(self.lead_annotator_extensions):

                    signal_one_lead = signals_all_leads[lead_idx, :]

                    try:
                        # Загружаем аннотации для текущего отведения, используя его имя как расширение
                        annotation = wfdb.rdann(record_path, extension=lead_name_lower)
                        mask_one_lead = self._create_full_mask(annotation.symbol, annotation.sample)

                        self.processed_samples.append((signal_one_lead.astype(np.float32), mask_one_lead))
                    except FileNotFoundError:
                        # Создание пустой маски
                        mask_one_lead = np.zeros(self.signal_len_points, dtype=np.int64)
                        self.processed_samples.append((signal_one_lead.astype(np.float32), mask_one_lead))
                    except Exception as e_ann:
                        print(f"Ошибка при чтении аннотаций для {record_name}.{lead_name_lower}: {e_ann}")
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            start_idx = np.random.randint(self.min_start_idx_fragment, self.max_start_idx_fragment + 1)
        else:
            start_idx = 1500
...
def calculate_segmentation_metrics(y_true_flat, y_pred_flat, class_labels, class_names_map):
    """
    ...
    metrics["overall_pixel_accuracy"] = accuracy_score(y_true_flat, y_pred_flat) * 100

    present_labels_in_true = np.unique(y_true_flat)
    labels_to_evaluate = [lbl for lbl in class_labels if lbl in present_labels_in_true]

    if not labels_to_evaluate:
        print("Внимание: в истинных метках (y_true_flat) нет указанных классов для детальной оценки.")
        for class_id in class_labels:
            ...
            metrics[f"{class_name}_pixel_accuracy"] = 0.0
            metrics[f"{class_name}_support"] = 0
        metrics["macro_f1_pqrst"] = 0.0
        return metrics

    precision, recall, f1, support = precision_recall_fscore_support(
        y_true_flat, y_pred_flat, labels=labels_to_evaluate, average=None, zero_division=0, beta=1.0
    )

    metrics["macro_f1_pqrst"] = f1_score(
        y_true_flat, y_pred_flat, labels=labels_to_evaluate, average="macro", zero_division=0
    )

    for i, class_id in enumerate(labels_to_evaluate):
        ...
        metrics[f"{class_name}_pixel_accuracy"] = recall[i] * 100

        metrics[f"{class_name}_support"] = support[i]

    for class_id in class_labels:
        class_name = class_names_map.get(class_id, f"Class_{class_id}")
        if f"{class_name}_f1" not in metrics:
            metrics[f"{class_name}_precision"] = 0.0
            metrics[f"{class_name}_recall"] = 0.0
            metrics[f"{class_name}_f1"] = 0.0
            metrics[f"{class_name}_pixel_accuracy"] = 0.0
...
def train_model(model_name_to_save="best_ecg_unet_model_f1_2", ludb_data_dir="ludb/data", num_epochs=100):
    ...
    CLASS_NAMES_MAP = {0: "Background", 1: "P", 2: "QRS", 3: "T"}
    TARGET_CLASS_LABELS_FOR_METRICS = [1, 2, 3]

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    if not os.path.isdir(LUDB_DATA_DIR):
        print(f"Ошибка: Директория {LUDB_DATA_DIR} не найдена. Укажите правильный путь.")
        return

    all_dat_files = [f for f in os.listdir(LUDB_DATA_DIR) if f.endswith(".dat")]
    available_record_names = sorted(list(set([f.split(".")[0] for f in all_dat_files])))

    if not available_record_names:
        print(f"В директории {LUDB_DATA_DIR} не найдены файлы .dat. Проверьте путь.")
        return
    ...
    print(f"Записей для обучения: {len(train_rec_names)}, для валидации: {len(val_rec_names)}")
    ...
    val_dataset = ECGDataset(data_dir_path=LUDB_DATA_DIR, record_names=val_rec_names, is_train=False)

    if len(train_dataset) == 0 or len(val_dataset) == 0:
        print("Один из датасетов пуст. Проверьте процесс загрузки данных, пути и наличие аннотаций.")
        return
    ...
    print(f"Размер обучающего датасета (отведения): {len(train_dataset)}, батчей: {len(train_loader)}")
    print(f"Размер валидационного датасета (отведения): {len(val_dataset)}, батчей: {len(val_loader)}")

    # --- Инициализация модели, функции потерь, оптимизатора ---
    model = UNetModel(in_channels=INPUT_CHANNELS, num_classes=NUM_CLASSES).to(device)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        with torch.no_grad():
            for batch in val_progress_bar:
                ...
                all_val_preds_epoch.append(predicted_classes.cpu().numpy().flatten())
        ...
        if len(all_val_preds_epoch) > 0:
            y_true_epoch = np.concatenate(all_val_masks_epoch)
            y_pred_epoch = np.concatenate(all_val_preds_epoch)

            val_metrics = calculate_segmentation_metrics(
                y_true_epoch, y_pred_epoch, TARGET_CLASS_LABELS_FOR_METRICS, CLASS_NAMES_MAP
            )

            print(f"  Overall Pixel Acc: {val_metrics['overall_pixel_accuracy']:.2f}%")
            print(f"  Macro F1 (P,QRS,T): {val_metrics['macro_f1_pqrst']:.4f}")
            for class_id in TARGET_CLASS_LABELS_FOR_METRICS:
                class_name = CLASS_NAMES_MAP.get(class_id, f"Class_{class_id}")
                print(
                    f"    {class_name}: "
                    f"Precision: {val_metrics[f'{class_name}_precision']:.4f}, "
                    f"Recall (PixelAcc): {val_metrics[f'{class_name}_recall']:.4f}, "
                    f"F1: {val_metrics[f'{class_name}_f1']:.4f}, "
                    f"Support: {val_metrics[f'{class_name}_support']}"
                )

            current_metric_to_save = val_metrics["macro_f1_pqrst"]
            if current_metric_to_save > best_val_metric:
                best_val_metric = current_metric_to_save
                torch.save(model.state_dict(), model_name_to_save + ".pth")
                print(f"Модель сохранена как {model_name_to_save}.pth (Val Macro F1: {best_val_metric:.4f})")
        else:
            print("Нет данных для валидации для расчета метрик.")
